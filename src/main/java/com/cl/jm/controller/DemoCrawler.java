...
public class DemoCrawler extends BreadthCrawler {
	...
	File baseDir = new File("E:\\images\\3d66");
	
	static String website = "https://www.3d66.com";
	...
	public void visit(Page page, CrawlDatums next) {
        ...
        if (contentType == null) {
            ...
            for (Element img : imgs) {
                ...
                if (imgSrc.indexOf("thumb") < 0 && imgSrc.startsWith("https://pic.3d66.com")) {
                	String id = "";
                	String name = "";
                	Elements idEle = page.select("b[id=clipboardId]");
                	if (!idEle.isEmpty()) {
                		id = idEle.first().text();
                		name = page.select("div[class='info-title clearfix']").first().child(0).text();
                		System.out.println("模型id="+id+",模型名字="+name);
                	}
                	imgSrc = imgSrc+"?id="+id+"&name="+name;
                    next.add(imgSrc);
            ...
            try {
                ...
                String imgName = page.url().split("\\?")[1];
                //根据图片MD5生成文件名
                String fileName = String.format("%s.%s", MD5Utils.md5(image)+imgName, extensionName);
